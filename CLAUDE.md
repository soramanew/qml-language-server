...
**Multiple producers, one list.** There are three external diagnostic producers (qmllint, the conventions script and the translation checker) and `publishDiagnostics` replaces the client's *entire* list for a URI, so no producer may publish on its own — whichever finished last would erase the others' findings. Every producer instead calls `h.setDiagnostics(uri, source, diags)`, which caches the result under that source key in `h.diagBySource` and publishes the union in `diagSources` order. Add a new producer by adding its source constant to `diagSources` and an entry to `Handler.diagProducers`, which describes each checker as a `{source, timeout, check}` triple that `startDiagnostics` fans out over; never call `publishDiagnostics` directly from a producer. `clearDiagnostics` (DidClose) drops the whole cache entry so a reopened file doesn't inherit stale findings.
...
### Project scripts
Two per-project Python scripts, if a workspace root ships them, feed diagnostics alongside qmllint. Both are found by path only — no contents or shebang check — so a project can swap in anything speaking the same flags. `handler/scriptcheck.go` holds what they share: `scriptLocator` (root search, and `checkBuffer`, which pipes the unsaved buffer in on stdin and hands back the stderr report), plus `scriptRange`, `reportedRange` and `ruleCode`. Both write their report to **stderr** — stdout is reserved for fixed source — and both exit non-zero on a finding, so the runners ignore exit status and key off parseable JSON.

**Positions.** Both scripts report a full range — `line`/`column`/`endLine`/`endColumn`, 1-based with an exclusive end, i.e. an LSP range in 1-based clothing — so both report types embed `scriptRange` and both runners convert it with `reportedRange`. Each script picks its own extent and we don't second-guess it: the conventions script spans the offending line's content (so the range skips the indent), trs-check spans the offending call. Earlier revisions of the scripts reported a line only, then a line and column; the whole-line and start-column-to-end-of-line helpers that compensated are gone.

`reportedRange` clamps to the buffer, which is defence against a report that raced an edit rather than distrust of the script, and it never drops a diagnostic for being out of range: a line the buffer no longer has falls back to the whole of the first line (the columns and end from that line are equally stale, so they're discarded with it — clamping the end independently would span the whole file), columns past the end of a line that shrank clamp to it, and a range that comes out empty or inverted widens to the end of its line, since a zero-width range renders as a bare caret.

#### Conventions script (`scripts/qml-lint-conventions.py`)
`handler/qmlconventions.go` drives it two different ways.
...
**Buffer check (`conventionsRunner.Check`).** Every diagnostic round runs `<script> --file - --json` and turns the report into warning-severity diagnostics under the `qml-conventions` source — every violation is a style nit, so there is no severity to map, unlike trs-check. We deliberately don't pass `--fix` here: with `--fix` the script reports against the source it rewrote, whose line numbers no longer match the buffer the editor is showing. The unfixed report therefore lists violations the post-save `--fix` will clear anyway; correct positions are worth that.

Both paths are strictly best-effort — missing script is a silent no-op, and a failing, silent, or non-JSON subprocess yields no diagnostics rather than an error. A broken convention tool must never block a save or lose another producer's diagnostics.

#### Translation checker (`scripts/trs-check.py`)
`handler/trscheck.go` catches translation bugs xgettext can't — a helper called with the wrong arity, a source string xgettext cannot extract, placeholders that skip a number, QML using `Tr` without importing `Caelestia.I18n`. Check-only: the script has no `--fix`, so there is no post-save counterpart. Every diagnostic round runs `<script> --file - --stdin-name <path> --json` under the `trs-check` source. `--stdin-name` matters — the script labels its issues with that path, uses the extension to tell QML from C++, and skips the files implementing the helpers themselves; without it the buffer is checked as an unnamed QML file. Its report carries a `level` per issue, mapped onto error (breaks the string for translators) and warning (merely suspicious) severity, plus the usual range (see **Positions** above); the script itself also checks `*.cpp`/`*.hpp`, which we never send it since we only serve QML buffers.
